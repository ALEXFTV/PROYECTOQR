...
def validarLetras(numero):
    ...
    for elemento in numero.lower():
        if elemento not in validos:
            raise ValidationError('Solo se Permite letras en el Campo Nombres ')
def validarLetrass(numero):
    validos = ['a', 'b',' ' ,',c','d','e','f','g','h','i','j','k','l','m','n','ñ','o','p','q','r','s','t','u','v','w','x','y','z']   # Una lista con todas las letras que quieras
    for elemento in numero.lower():
        if elemento not in validos:
            raise ValidationError('Solo se Permite letras en el Campo Apellidos')
def validarLetrasss(numero):
    validos = ['a', 'b',' ' ,',c','d','e','f','g','h','i','j','k','l','m','n','ñ','o','p','q','r','s','t','u','v','w','x','y','z']   # Una lista con todas las letras que quieras
    for elemento in numero.lower():
        if elemento not in validos:
            raise ValidationError('Solo se Permite letras en el Campo Direccion')

def validacionTelefono(numero):
    validos = ['1','2','3','4','5','6','7','8','9','0']
    for elemento in numero.lower():
        if elemento not in validos:
            raise ValidationError('Ingrese solo numeros..!!')
